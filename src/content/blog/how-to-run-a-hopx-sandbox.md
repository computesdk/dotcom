---
title: "How to run a Hopx sandbox"
...
## Why use Hopx as your sandbox provider?

- Hopx provides secure, isolated Linux micro-VM sandboxes with millisecond startup times.
- They are purpose-built for AI coding agents that need to safely execute untrusted code.
- Hopx offers robust security boundaries ideal for running and testing AI-generated code at scale.

**Let's see how we can easily run a basic Vite app inside of a Hopx sandbox.**
...
```bash
npx create-next-app@latest hopx-basic
```
...
### Install ComputeSDK and the HopX provider
...
## Create or log in to your Hopx account
<!-- markdownlint-disable-next-line MD033 -->
Create a Hopx account or log in <a href="https://hopx.ai" target="_blank">here</a>.\
Once you have created an account, you'll need to get your Hopx API key.\
Go to your dashboard and create an API key.

Save your API key in your `.env` file to the `HOPX_API_KEY` variable.
...
Import the `hopx` factory from `@computesdk/hopx` and pass it your API key. `compute.sandbox.create()` provisions a sandbox on Hopx.
...
Then check your Hopx dashboard.\
You should see a new sandbox created!

Success!

## You've successfully created your first Hopx sandbox
...
// Custom vite.config.js to allow access to sandbox at port 5173
  ...
  export default defineConfig({
    ...
    server: {
      ...
      strictPort: true,
      hmr: false,
      allowedHosts: ['.hopx.ai', 'localhost', '127.0.0.1'],
    },
  })
  `;
...
`cwd` is an optional per-call override — if you don't pass one, commands run in whatever HopX's own sandbox default working directory is. We pass `cwd: 'app'` here simply because that's the subfolder we just scaffolded the Vite project into.
...
HopX resolves this through its own preview URL API — check your terminal's console output for the exact domain it returns; it's HopX's own domain, not a shared ComputeSDK one.
...
<!-- markdownlint-disable-next-line MD033 -->
<img style="margin: 12px auto; border-radius: 10px;" width="700px" src="/sandbox-vite-app-in-browser.png" alt="screenshot of Vite app running in Hopx sandbox via ComputeSDK" title="Basic Vite App in Hopx sandbox" />

## Congrats! You've successfully created your first sandbox application

You have done the following:

- created a Hopx sandbox with ComputeSDK
...
So now that you've written this code in Hopx, you can easily adjust this code to run in any sandbox provider.
