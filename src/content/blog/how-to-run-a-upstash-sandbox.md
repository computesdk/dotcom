---
title: "How to run a Upstash sandbox"
description: "A step-by-step process for creating a sandbox with Upstash, running a basic Vite app inside, and accessing it securely via the browser."
date: "2026-07-13"
tags: [how-to, sandboxes, upstash]
author: "David Tice"
...
<span style="font-size: 14px; font-style: italic;">You can clone <a href="https://github.com/computesdk/examples/tree/main/upstash-basic" target="_blank">this repo</a> and update your credentials to run locally.</span>

Upstash Box provides cloud sandboxes with two variants: a full-featured Default Box, and a lightweight Ephemeral Box for short-lived tasks.
Let's walk through the process of getting a basic application running inside a Upstash sandbox.

## Why use Upstash as your sandbox provider?

- Two box types: a full-featured Default Box (filesystem, shell, snapshots, preview URLs), or a lightweight Ephemeral Box for short-lived, one-off tasks.
- Backed by Upstash's existing developer platform, so it's an easy add if you already use Upstash.
- A single API key is all you need to get started.

**Let's see how we can easily run a basic Vite app inside of a Upstash sandbox.**
...
```bash
npx create-next-app@latest upstash-basic
```
...
### Install ComputeSDK and the Upstash provider
...
## Create or log in to your Upstash account
<!-- markdownlint-disable-next-line MD033 -->
Create a Upstash account or log in <a href="https://upstash.com" target="_blank">here</a>.\
Create an account, then generate an API key from your Upstash console under Box.
...
Import the `upstash` factory from `@computesdk/upstash` and pass it your credentials. `compute.sandbox.create()` provisions a sandbox on Upstash.\
...
Then check your Upstash console.\
You should see a new sandbox created!

Success!

## You've successfully created your first Upstash sandbox
...
// Custom vite.config.js to allow access to sandbox at port 5173
  ...
  export default defineConfig({
    ...
    server: {
      ...
      strictPort: true,
      hmr: false,
      allowedHosts: ['.preview.box.upstash.com', 'localhost', '127.0.0.1'],
    },
  })
  `;
...
Upstash's Default Box preview URLs follow the pattern `https://<box-id>-<port>.preview.box.upstash.com` (Ephemeral Boxes don't support preview URLs at all) — an Upstash domain, not a shared ComputeSDK one.
...
Now, after you click the "Create Upstash Sandbox" button on your localhost homepage you should:

1. See a new sandbox created in your Upstash console.
2. See a preview URL logged to your terminal output.
3. Finally, if you visit that URL you should see the boilerplate Vite React app running in your Upstash sandbox!

<!-- markdownlint-disable-next-line MD033 -->
<img style="margin: 12px auto; border-radius: 10px;" width="700px" src="/sandbox-vite-app-in-browser.png" alt="screenshot of Vite app running in Upstash sandbox via ComputeSDK" title="Basic Vite App in Upstash sandbox" />

## Congrats! You've successfully created your first sandbox application

You have done the following:

- created a Upstash sandbox with ComputeSDK
...
So now that you've written this code for Upstash, you can easily adjust this code to run in any sandbox provider.
