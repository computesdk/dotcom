---
title: "How to run a Lightning sandbox"
description: "A step-by-step process for creating a sandbox with Lightning, running a basic Vite app inside, and accessing it securely via the browser."
date: "2026-07-13"
tags: [how-to, sandboxes, lightning]
author: "David Tice"
...
<span style="font-size: 14px; font-style: italic;">You can clone <a href="https://github.com/computesdk/examples/tree/main/lightning-basic" target="_blank">this repo</a> and update your credentials to run locally.</span>

Lightning AI provides cloud sandboxes with curated runtime images, optional persistence, and spot capacity.
Let's walk through the process of getting a basic application running inside a Lightning sandbox.

## Why use Lightning as your sandbox provider?

- Curated runtime images (node24, python313, and more) so your sandbox matches your stack out of the box.
- Optional persistent sandboxes that auto-snapshot filesystem state across stops.
- Spot capacity support for cost-efficient, non-critical workloads.

**Let's see how we can easily run a basic Vite app inside of a Lightning sandbox.**
...
```bash
npx create-next-app@latest lightning-basic
```
...
### Install ComputeSDK and the Lightning provider
...
## Create or log in to your Lightning account
<!-- markdownlint-disable-next-line MD033 -->
Create a Lightning account or log in <a href="https://lightning.ai" target="_blank">here</a>.\
Create an account, then generate an API key from your Lightning AI dashboard. This provider requires Node.js 22+.
...
Import the `lightning` factory from `@computesdk/lightning` and pass it your credentials. `compute.sandbox.create()` provisions a sandbox on Lightning.\
...
Then check your Lightning AI dashboard.\
You should see a new sandbox created!

Success!

## You've successfully created your first Lightning sandbox
...
Lightning returns its own public HTTPS domain for the port — not a shared ComputeSDK one. This only works because we declared `ports: [5173]` when we created the sandbox above; Lightning sandboxes throw on `getUrl()` for a port that wasn't declared at creation time. We don't have a fixed domain to pin down here — add it to the `allowedHosts` array above once you see it in your terminal output.
...
Now, after you click the "Create Lightning Sandbox" button on your localhost homepage you should:

1. See a new sandbox created in your Lightning AI dashboard.
2. See a preview URL logged to your terminal output.
3. Finally, if you visit that URL you should see the boilerplate Vite React app running in your Lightning sandbox!

<!-- markdownlint-disable-next-line MD033 -->
<img style="margin: 12px auto; border-radius: 10px;" width="700px" src="/sandbox-vite-app-in-browser.png" alt="screenshot of Vite app running in Lightning sandbox via ComputeSDK" title="Basic Vite App in Lightning sandbox" />

## Congrats! You've successfully created your first sandbox application

You have done the following:

- created a Lightning sandbox with ComputeSDK
...
So now that you've written this code for Lightning, you can easily adjust this code to run in any sandbox provider.
