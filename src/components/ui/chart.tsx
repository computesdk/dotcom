...
>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      if (labelFormatter) {
        return (
          <div className="font-medium">{labelFormatter(value as string, payload)}</div>
        )
      }
      ...
      >
        ...
        <div className="grid gap-1.5">
          {sortedPayload.map((item: any, index: number) => {
            ...
            const indicatorColor = item.color || (itemConfig?.color as string)
      ...
      >
        {payload.map((item: any) => {
          ...
          return (
            <div
              key={item.value}
              className="flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-gray-500 dark:[&>svg]:text-gray-400"
            >
