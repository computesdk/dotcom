...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Cell, Tooltip, LineChart, Line, Legend } from "recharts"
...
const STORAGE_METRIC_LABELS: Record<StorageMetric, string> = {
...
function IterationHistogram({
  ...
  const { buckets } = useMemo(() => {
    ...
    for (let i = 0; i < BUCKET_COUNT; i++) {
      ...
      result.push({
        label: unit === "Mbps" ? `${rangeStart.toFixed(0)}` : `${(rangeStart / 1000).toFixed(2)}s`,
        count,
      })
    }
...
export function StorageProviderDetailDashboard({
  provider,
  result,
  historyData,
  ...
  const uploadValues = useMemo(
    () => result.iterations?.filter((i) => !i.error).map((i) => i.uploadMs) ?? [],
    [result.iterations]
  )
  const downloadValues = useMemo(
    () => result.iterations?.filter((i) => !i.error).map((i) => i.downloadMs) ?? [],
    [result.iterations]
  )
  const throughputValues = useMemo(
    () => result.iterations?.filter((i) => !i.error).map((i) => i.throughputMbps) ?? [],
    [result.iterations]
  )
          ...
          return (
            <>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-base font-semibold text-gray-900 dark:text-white m-0">10 MB Transfer</h2>
              ...
              <LineChart data={historyData} margin={{ top: 10, right: 20, left: 10, bottom: 10 }}>
                ...
                <YAxis
                  ...
                  tickFormatter={(v: number) => {
                    if (selectedMetric === "uploadMs") return v.toFixed(0)
        ...
        <div className="border-t border-gray-200/50 dark:border-gray-700/50">
          <div className="md:max-w-7xl md:mx-auto px-4 md:px-6 py-6 md:py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Upload Latency</h3>
                <IterationHistogram values={uploadValues} color={color} unit="ms" />
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Download Latency</h3>
                <IterationHistogram values={downloadValues} color={color} unit="ms" />
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">Throughput</h3>
                <IterationHistogram values={throughputValues} color={color} unit="Mbps" higherBetter />
              </div>
            </div>
          </div>
        </div>
